...
import type {
  ResponseObject,
  ResponsesObject,
} from "@omer-x/openapi-types/response";
...
function maskContentSchema(
  storedSchemas: Record<string, ZodType>,
  bodyContent?: Record<string, MediaTypeObject>,
) {
...
function maskRequestBodySchema(
  storedSchemas: Record<string, ZodType>,
...
function maskSchemasInResponses(
  storedSchemas: Record<string, ZodType>,
...
export default function maskOperationSchemas(
  ...
  return {
    ...
    parameters: operation.parameters?.map((p) =>
      maskParameterSchema(p, storedSchemas),
    ),
